...
* Users build a computational object, called an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++], by selecting the computations in which they are interested, or authoring their own computational primitives which fit within the framework.
* Users push data into the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] object one sample at a time.
* The xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] computes the requested quantities in the most efficient method possible, resolving dependencies between requested calculations, possibly caching intermediate results.

The Accumulators Framework defines the utilities needed for defining primitive computational elements, called _accumulators_. It also provides the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] type, described above.
...
Sample::
  [[sample_type]] A datum that is pushed into an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++]. The type of the sample is the _sample type_.
...
Feature::
  An abstract primitive computational entity. When defining an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++], users specify the features in which they are interested, and the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] figures out which _accumulators_ would best provide those features. Features may depend on other features. If they do, the accumulator set figures out which accumulators to add to satisfy the dependencies.
...
Extractor::
  A function or function object that can be used to extract a result from an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++].
...
| xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] | This is the most important type in the Accumulators Framework. It is a collection of accumulators. A datum pushed into an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] is forwarded to each accumulator, in an order determined by the dependency relationships between the accumulators. Computational results can be extracted from an accumulator at any time.

| xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>depends_on&lt;&gt;</code></em>+++] | Used to specify which other features a feature depends on.

| xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>feature_of&lt;&gt;</code></em>+++] | Trait used to tell the Accumulators Framework that, for the purpose of feature-based dependency resolution, one feature should be treated the same as another.

| xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>as_feature&lt;&gt;</code></em>+++] | Used to create an alias for a feature. For example, if there are two features, fast_X and accurate_X, they can be mapped to X(fast) and X(accurate) with xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>as_feature&lt;&gt;</code></em>+++]. This is just syntactic sugar.

| xref:reference.adoc#doxygen.accumulators_framework_reference.features_8hpp[+++<em><code>features&lt;&gt;</code></em>+++] | An link:../../libs/mpl/index.html[MPL] sequence. We can use xref:reference.adoc#doxygen.accumulators_framework_reference.features_8hpp[+++<em><code>features&lt;&gt;</code></em>+++] as the second template parameter when declaring an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++].

| xref:reference.adoc#+++doxygen.accumulators_framework_reference.external__accumulator_8hpp+++[+++<em><code>external&lt;&gt;</code></em>+++] | Used when declaring an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++]. If the weight type is specified with xref:reference.adoc#+++doxygen.accumulators_framework_reference.external__accumulator_8hpp+++[+++<em><code>external&lt;&gt;</code></em>+++], then the weight accumulators are assumed to reside in a separate accumulator set which will be passed in with a named parameter.

| +++<em><code>extractor&lt;&gt;</code></em>+++ | A class template useful for creating an extractor function object. It is parameterized on a feature, and it has member functions for extracting from an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] the result corresponding to that feature.
...
Our tour of the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] class template begins with the forward declaration:
...
For example, the following line declares an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] that will accept a sequence of doubles one at a time and calculate the min and mean:
...
Notice that we use the xref:reference.adoc#doxygen.accumulators_framework_reference.features_8hpp[+++<em><code>features&lt;&gt;</code></em>+++] template to specify a list of features to be calculated. xref:reference.adoc#doxygen.accumulators_framework_reference.features_8hpp[+++<em><code>features&lt;&gt;</code></em>+++] is an MPL sequence of features.
...
Once we have defined an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++], we can then push data into it, and it will calculate the quantities you requested, as shown below.
...
Since xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] defines its accumulate function to be the function call operator, we might be tempted to use an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] as a UnaryFunction to a standard algorithm such as `std::for_each`. That's fine as long as we keep in mind that the standard algorithms take UnaryFunction objects by value, which involves making a copy of the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] object. Consider the following:
...
Once we have declared an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] and pushed data into it, we need to be able to extract results from it. For each feature we can add to an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++], there is a corresponding extractor for fetching its result. Usually, the extractor has the same name as the feature, but in a different namespace. For example, if we accumulate the `tag::min` and `tag::max` features, we can extract the results with the `min` and `max` extractors, as follows:
...
Another way to extract a result from an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] is with the `extract_result()` function. This can be more convenient if there isn't an extractor object handy for a certain feature. The line above which displays results could equally be written as:
...
To declare an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] that accepts weighted samples, you must specify the type of the weight parameter as the 3rd template parameter, as follows:
...
When you specify a weight, all the accumulators in the set are replaced with their weighted equivalents. For example, the above xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] declaration is equivalent to the following:
...
Some accumulators depend on other accumulators within the same accumulator set. In those cases, it is necessary to be able to access those other accumulators. To make this possible, the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] passes a reference to itself when invoking the member functions of its contained accumulators. It can be accessed by using the special `accumulator` key with the argument pack. Consider how we might implement `mean_accumulator`:
...
The term "droppable" refers to an accumulator that can be removed from the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++]. You can request that an accumulator be made droppable by using the xref:reference.adoc#+++doxygen.accumulators_framework_reference.droppable__accumulator_8hpp+++[+++<em><code>droppable&lt;&gt;</code></em>+++] class template.
...
Once we have implemented an accumulator, we must define a feature for it so that users can specify the feature when declaring an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++]. We typically put the features into a nested namespace, so that later we can define an extractor of the same name. All features must satisfy the xref:user_s_guide.adoc#accumulators.user_s_guide.the_accumulators_framework.concepts.feature_concept[Feature Concept]. Using xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>depends_on&lt;&gt;</code></em>+++] makes satisfying the concept simple. Below is an example of a feature definition.
...
All features must also provide a nested `is_weight_accumulator` typedef. It must be either `mpl::true_` or `mpl::false_`. xref:reference.adoc#+++doxygen.accumulators_framework_reference.depends__on_8hpp+++[+++<em><code>depends_on&lt;&gt;</code></em>+++] provides a default of `mpl::false_` for all features that inherit from it, but that can be overridden (or hidden, technically speaking) in the derived type. When the feature represents an accumulation of information about the weights instead of the samples, we can mark this feature as such with `typedef mpl::true_ is_weight_accumulator;`. The weight accumulators are made external if the weight type is specified using the xref:reference.adoc#+++doxygen.accumulators_framework_reference.external__accumulator_8hpp+++[+++<em><code>external&lt;&gt;</code></em>+++] template.
...
Once defined, the `mean` extractor can be used to extract the result of the `tag::mean` feature from an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++].
...
The return type deserves some explanation. Every xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] type is actually a unary link:../../libs/mpl/doc/refmanual/metafunction-class.html[MPL Metafunction Class]. When you `mpl::apply<>` an xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>accumulator_set&lt;&gt;</code></em>+++] and a feature, the result is the type of the accumulator within the set that implements that feature. And every accumulator provides a nested `result_type` typedef that tells what its return type is. The extractor simply delegates its work to the xref:reference.adoc#+++doxygen.accumulators_framework_reference.accumulator__set_8hpp+++[+++<em><code>extract_result()</code></em>+++] function.
